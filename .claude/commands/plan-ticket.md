...
1. `gh issue view N` — read the whole ticket, including comments (the real scoping often lives in
   the comment thread, not the body).
...
3. If the ticket names a design spec in `docs/superpowers/specs/`, read it — that spec is the
   owner-approved decision record, and the plan implements it rather than re-deciding it.
4. Read **only** the files the ticket points at, plus what they force you to read. Resist a broad
   codebase tour; a plan written from a bloated context is a worse plan.
5. Use the `superpowers:writing-plans` skill. Save to `docs/superpowers/plans/YYYY-MM-DD-issue-N.md`.

## Where this project's plans go wrong

- **The Teams message layout is not in this repo.** It lives in the owner's Power Automate flow.
  A plan that changes what the alert looks like must call out the flow edit as a separate,
  owner-performed step and say exactly what HTML to paste.
- **There is no test suite**, so "write the failing test first" doesn't apply. The plan's
  verification steps are `python -m py_compile src/*.py`, `./scripts/send-test.sh`, and a real
  `python -m src.run` cycle. Say which one proves which task.
